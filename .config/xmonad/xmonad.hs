...
myStartupHook = do
  ...
  spawnOnce "/usr/lib/aarch64-linux-gnu/libexec/polkit-kde-authentication-agent-1"
  spawnOnce "picom"
  spawnOnce "nm-applet"
...
myWorkspaces = [" dev ", " www ", " sys ", " doc ", " file ", " mus ", " pho ", " vid ", " gfx "]
...
myKeys c =
  ...
  , ("M-S-z", addName "Quit XMonad"            $ sequence_ [spawn (mySoundPlayer ++ shutdownSound), io exitSuccess])
  ...
  ^++^ subKeys "Emacs"
  [ ("M-e e", addName "Emacsclient Dashboard"    $ spawn "emacsclient -c -a ''")
